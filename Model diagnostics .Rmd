...
sum(duplicated(cancer_raw), na.rm = TRUE)
...
Add Scale to the model data for those non-categorical variable:
...
---plots of our Model diagnostics

o Heteroscedasticity, normality and multicollinearity
o Functional form for continuous predictors
• Outliers and missing values

## Dignostic plots: stepwise
...
Heteroscedasticity: The residuals vs. Fitted value informs a good value bounce around 0, and it indicates the assumptions are not violated for equal variance. 

For both scaled data(only for continuous) and unscaled data, we found that there is no outliers in the data. 
...
In our example, the highest VIF score for the predictor variable pct_public_coverage is not high (VIF = 2.648187). This might not be problematic.
